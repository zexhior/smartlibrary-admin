...
const UpdateBook: React.FC<UpdateBookProps> = ({ action }) => {
  ...
  const { categories, setCategories, lastCategories } =
    GetCategoriesByClassification(book?._id);
  const { authors, setAuthors, lastAuthors } = GetAuthorsByWork(book?._id);
  const navigate = useNavigate();
  const [delCategories, setDelCategories] = useState<Array<string>>(
    new Array<string>()
  );
  ...
  const HandlerSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (action === "create") {
      ...
      categories.forEach((data: any) => {
        CreateOneElement("classifications", {
          book: new_book._id,
    ...
    <div className="updatebook">
      ...
      <form onSubmit={(e) => HandlerSubmit(e)}>
        ...
        <CategoryOfBook
          ...
          setDelCategories={setDelCategories}
